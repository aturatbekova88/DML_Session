...
insert into people (id, first_name, last_name, email, gender, age, language)
values (51, 'Aidana', 'Turatbekova', 'aidana@mail.com', 'Female', 18, 'Kyrgyz');
...
-- 11. Вывести имя и год рождение людей которые говорят на английском языке
...
select concat(first_name,' ',last_name)as full_name ,gender, age  from people where age < 25 and gender = 'Male';
-- 15. Посчитать сколько всего имен из таблицы начинаются на букву А
select count(*) from people where first_name like 'A%';
-- 16. Отсартируйте все почты в алфавитном порядке
...
select count(*) from people group by language;
...
select language ,count(id) from people where gender = 'Female' group by language order by count(*) desc limit 1;
-- 22. Обновить язык для людей, чей возраст между 3 и 10 годами, на 'French'
update people set language = 'French' where age > 3 and age < 10;
...
select * from people where email like ('%.com') order by age limit 5;
...
select * from  people where (first_name ilike '%a%' or last_name ilike '%a%') and age between 20 and 56;
